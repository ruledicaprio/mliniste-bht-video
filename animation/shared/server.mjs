// Minimal static file server rooted at the repo, so a piece's page can fetch
// route.json and the SVG backdrop over http:// — file:// blocks both under CORS.
//
// Shared by every piece: the root is the repo, so intro.html and outro.html are
// both reachable, and each piece's own server.mjs only supplies its page path.
...
/** Repo-relative URL of a piece's page, e.g. pagePath('intro') -> /animation/intro/intro.html */
export const pagePath = (piece) => `/animation/${piece}/${piece}.html`;
...
/** Starts a preview server and prints the URL of one piece's page. */
export async function preview(piece) {
  const s = await startServer();
  console.log(`preview: ${s.origin}${pagePath(piece)}`);
